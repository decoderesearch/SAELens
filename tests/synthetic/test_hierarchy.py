...
class TestComputeProbabilityCorrectionFactors:
    ...
    def test_correction_recovers_original_rates_with_mutual_exclusion(self):
        """Verify correction works for hierarchies with mutually exclusive children.

        With ME children, the correction must account for:
        1. Parent deactivation (same as without ME)
        2. ME selection probability (when multiple siblings are active, one is chosen)

        The effective rate P(child fires after ME) = corrected × parent_prob × win_prob,
        where win_prob is the probability of winning the ME selection.
        """
        from sae_lens.synthetic import Hierarchy

        # Create hierarchy with ME children:
        # Tree 1: Root (0) -> ME children (1, 2, 3)
        # Tree 2: Root (4) -> Child (5) -> ME grandchildren (6, 7)
        # Feature outside hierarchy (8)
        me_child1 = HierarchyNode(feature_index=1)
        me_child2 = HierarchyNode(feature_index=2)
        me_child3 = HierarchyNode(feature_index=3)
        tree1 = HierarchyNode(
            feature_index=0,
            children=[me_child1, me_child2, me_child3],
            mutually_exclusive_children=True,
        )

        me_gc1 = HierarchyNode(feature_index=6)
        me_gc2 = HierarchyNode(feature_index=7)
        child = HierarchyNode(
            feature_index=5,
            children=[me_gc1, me_gc2],
            mutually_exclusive_children=True,
        )
        tree2 = HierarchyNode(feature_index=4, children=[child])

        hierarchy = Hierarchy(
            roots=[tree1, tree2], modifier=hierarchy_modifier([tree1, tree2])
        )

        # Use probabilities chosen so corrected probs don't exceed 1.0
        # For ME children, the sum of base probs should be <= parent base prob
        # Tree 1: base[0]=0.9, ME children sum = 0.15+0.2+0.25 = 0.6 <= 0.9 ✓
        # Tree 2: base[4]=0.9, base[5]=0.8, ME gc sum = 0.2+0.3 = 0.5 <= 0.8 ✓
        base_probs = torch.tensor([0.9, 0.15, 0.2, 0.25, 0.9, 0.8, 0.2, 0.3, 0.5])
        correction = hierarchy.compute_probability_correction_factors(base_probs)
        ...
        # Apply hierarchy constraints (including ME)
        assert hierarchy.modifier is not None
        result = hierarchy.modifier(activations)

        # Verify ME is enforced
        # Tree 1 ME children (1, 2, 3): at most one active
        me1_active = (result[:, 1] > 0).int() + (result[:, 2] > 0).int()
        me1_active += (result[:, 3] > 0).int()
        assert (me1_active <= 1).all(), "Tree 1 ME should have at most one child active"

        # Tree 2 ME grandchildren (6, 7): at most one active
        me2_active = (result[:, 6] > 0).int() + (result[:, 7] > 0).int()
        assert (me2_active <= 1).all(), "Tree 2 ME should have at most one gc active"
