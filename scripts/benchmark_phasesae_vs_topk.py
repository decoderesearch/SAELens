...
def run_benchmark():
    ...
    results = {
        ...
        "training_tokens": training_tokens,
        "topk": {
            "ce_loss_score": topk_metrics["model_performance_preservation"].get("ce_loss_score", 0.0),
            "kl_div_score": topk_metrics["model_behavior_preservation"].get("kl_div_score", 0.0),
            "explained_variance": topk_metrics["reconstruction_quality"].get("explained_variance", 0.0),
        ...
        "phase_sae": {
            "num_phases": 4,
            "k_per_phase": 4,
            "ce_loss_score": phase_metrics["model_performance_preservation"].get("ce_loss_score", 0.0),
            "kl_div_score": phase_metrics["model_behavior_preservation"].get("kl_div_score", 0.0),
            "explained_variance": phase_metrics["reconstruction_quality"].get("explained_variance", 0.0),
    ...
    print("-" * 75)
    print(f"{'CE Loss Score (% Recovered)':<35} | {results['topk']['ce_loss_score']*100:>16.2f}% | {results['phase_sae']['ce_loss_score']*100:>16.2f}%")
    print(f"{'KL Divergence Score':<35} | {results['topk']['kl_div_score']:>17.4f} | {results['phase_sae']['kl_div_score']:>17.4f}")
    print(f"{'Explained Variance (R^2)':<35} | {results['topk']['explained_variance']:>17.4f} | {results['phase_sae']['explained_variance']:>17.4f}")
    print(f"{'MSE Reconstruction Error':<35} | {results['topk']['mse']:>17.4f} | {results['phase_sae']['mse']:>17.4f}")
    print(f"{'L0 (Active Latents)':<35} | {results['topk']['l0']:>17.2f} | {results['phase_sae']['l0']:>17.2f}")
    print(f"{'Peak Stream Param Footprint':<35} | {results['topk']['peak_stream_param_slice_mb']:>14.2f} MB | {results['phase_sae']['peak_stream_param_slice_mb']:>14.2f} MB (1/4)")
    print("=" * 70)
